...
namespace Physics {
	namespace External {
...
#if WIND_ACTIVE							  //6.253, 0.0, 0.0 - a constant 14mph wind due east
		glm::dvec3 Environment::windVelocity_m_per_s = glm::dvec3(0.0, -1.0, 0.0);
#else
		glm::dvec3 Environment::windVelocity_m_per_s = glm::dvec3(0.0);
#endif
		...
		double Environment::toGeopotential_m(int geometricAltitude_m) {
			return (::External::Earth::WGS84::a * geometricAltitude_m) / (::External::Earth::WGS84::a + geometricAltitude_m);
		}

		double Environment::toGeometric_m(int geopotentialAltitude_m) {
			return (::External::Earth::WGS84::a * geopotentialAltitude_m) / (::External::Earth::WGS84::a - geopotentialAltitude_m);
		}
			...
			//http://www.braeunig.us/space/atmmodel.htm
		{
			...
			if (altitude_m > 1'000'000.0)
				return 0.00000001; //The vacuum of space has virtually no air pressure
		...
		double Environment::getGravity(int altitude_m) {
			...
			return seaLevelGravity * pow(::External::Earth::WGS84::a / (::External::Earth::WGS84::a + altitude_m), 2.0);
			//return 9.80665;
		}
